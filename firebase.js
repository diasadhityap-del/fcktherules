...
export async function getOrders() {
    ...
    } catch (err) { console.error("Gagal ambil order:", err); return []; }
}

export async function updateOrderStatus(orderId, status) {
    try { await updateDoc(doc(db, "orders", orderId), { status }); return true; }
...
export async function updateProduk(id, data) { await updateDoc(doc(db, "produk", id), data); return true; }
export async function deleteProduk(id) { await deleteDoc(doc(db, "produk", id)); return true; }
...
export async function saveGaleri(url) {
    await addDoc(collection(db, "galeri"), { url, order: Date.now(), createdAt: new Date().toISOString() });
    return true;
}
...
export async function deleteGaleri(id) { await deleteDoc(doc(db, "galeri", id)); return true; }
export async function updateGaleri(id, data) { await updateDoc(doc(db, "galeri", id), data); return true; }
...
/* ============== BANNER ============== */
export async function saveBanner(data) { await addDoc(collection(db, "banners"), { ...data, createdAt: new Date().toISOString() }); return true; }
export async function updateBanner(id, data) { await updateDoc(doc(db, "banners", id), data); return true; }
export async function deleteBanner(id) { await deleteDoc(doc(db, "banners", id)); return true; }
...
export async function saveBannerText(data) { await setDoc(doc(db, "settings", "bannerText"), data); }

/* ============== VOUCHER ============== */
export async function saveVoucher(data) { await addDoc(collection(db, "vouchers"), { ...data, createdAt: new Date().toISOString() }); return true; }
export async function deleteVoucher(id) { await deleteDoc(doc(db, "vouchers", id)); return true; }
