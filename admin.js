...
let memberDeleteConfirm = new Set();
let pembeliDeleteConfirm = new Set();
...
function renderMemberTab() {
    ...
    list.innerHTML = data.map(c => {
        ...
        const inConfirm = memberDeleteConfirm.has(c.email.toLowerCase());
        const lastLogin = formatTanggalWaktu(c.lastLoginAt) || 'Belum ada data login';

        const deleteArea = inConfirm
            ? `<button onclick="event.stopPropagation(); confirmDeleteCustomer('member','${c.email}')" title="Konfirmasi hapus" style="width:34px; height:34px; border:none; background:var(--red); color:#fff; border-radius:10px; cursor:pointer; font-weight:900; font-size:16px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">&times;</button>`
            : `<button onclick="event.stopPropagation(); toggleDeleteConfirm('member','${c.email}')" title="Hapus" class="btn-icon del" style="width:34px; height:34px; border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0;"><i class="fas fa-trash"></i></button>`;
...
function renderPembeliTab() {
    ...
    list.innerHTML = data.map(c => {
        ...
        const inConfirm = pembeliDeleteConfirm.has(c.email.toLowerCase());

        const trailingArea = inConfirm
            ? `<button onclick="event.stopPropagation(); confirmDeleteCustomer('pembeli','${c.email}')" title="Konfirmasi hapus" style="width:34px; height:34px; border:none; background:var(--red); color:#fff; border-radius:10px; cursor:pointer; font-weight:900; font-size:16px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">&times;</button>`
            : `<span style="font-size:13px; font-weight:700; color:var(--green); min-width:20px; text-align:right;">${c.purchaseCount}</span>
               <button onclick="event.stopPropagation(); toggleDeleteConfirm('pembeli','${c.email}')" title="Hapus" class="btn-icon del" style="width:34px; height:34px; border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0;"><i class="fas fa-trash"></i></button>`;
...
window.toggleDeleteConfirm = (tab, email) => {
    const set = tab === 'member' ? memberDeleteConfirm : pembeliDeleteConfirm;
    const key = email.toLowerCase();
    if (set.has(key)) set.delete(key); else set.add(key);
    if (tab === 'member') renderMemberTab(); else renderPembeliTab();
};
...
window.confirmDeleteCustomer = async (tab, email) => {
    ...
    try {
        ...
        memberDeleteConfirm.delete(key);
        pembeliDeleteConfirm.delete(key);
        renderCustomers();
